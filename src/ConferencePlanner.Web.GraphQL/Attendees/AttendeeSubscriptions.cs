...
public class AttendeeSubscriptions
{
    ...
    public SessionAttendeeCheckIn OnAttendeeCheckedIn(
        ...
        CancellationToken cancellationToken) => new SessionAttendeeCheckIn(attendeeId, sessionId);

    public async ValueTask<ISourceStream<int>> SubscribeToOnAttendeeCheckedInAsync(
        int sessionId,
        [Service] ITopicEventReceiver eventReceiver,
        CancellationToken cancellationToken) => await eventReceiver.SubscribeAsync<string, int>($"OnAttendeeCheckedIn_{sessionId}", cancellationToken);
}
